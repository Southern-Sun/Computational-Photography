...
def get_equirectangular_image(reflection_vector, hdr_image):
    ...
    for c in range(C):
        equirectangular_intensity = griddata(
            spherical_coord, spherical_vals[:, c], equirectangular_coord
        )
    ...
    equirectangular_image[equirectangular_image != equirectangular_image] = (
        equirectangular_image[equirectangular_image == equirectangular_image].mean()
    )
